...
Implemented and passing. Branch `virtual-zarr` in `vendor/duckdb-zarr`, one commit `ca35cdd` on top of upstream `main` (`885f8dd`), pushed to the fork `kentstephen/duckdb-zarr` (remote `fork`). PR https://github.com/xqlsystems/duckdb-zarr/pull/46 is ready for review with all findings in the description. zarrs fletcher32 bug filed as https://github.com/zarrs/zarrs/issues/460. virtual-tiff and VirtualiZarr reports not filed. The same diff is exported to `patches/duckdb-zarr/virtual-zarr-0001.patch`. Rust unit tests 22/22, `make lint` clean, all six SQL suites pass (`test/sql/read_zarr_kerchunk.test` is new).
